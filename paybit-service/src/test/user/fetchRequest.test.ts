...
beforeAll(async () => {
  ...
  await User.deleteMany({ email: { $in: [requesterUser.email, senderUser.email] } });
  ...
  const hashedRequesterPassword = await bcrypt.hash(requesterUser.password, salt);
  ...
  it('should handle server errors gracefully', async () => {
    ...
    await request(app)
      ...
      .expect((res) => {
        ...
        expect(res.body).toHaveProperty('message', 'Server error fetching requests');
      });
