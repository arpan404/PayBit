...
describe('User - Send Money Request Controller', () => {
  ...
  beforeAll(async () => {
    ...
    await User.deleteMany({ email: { $in: [requester.email, recipient.email] } });
  ...
  it('should successfully send a money request', async () => {
    ...
    expect(response.body).toHaveProperty('message', 'Money request sent successfully');
    ...
    expect(moneyRequest.requester).toHaveProperty('fullname', requester.fullname);
  ...
  it('should preserve fields during create and retrieve', async () => {
    ...
    // Check timestamps
    expect(new Date(savedRequest?.createdAt as Date).getTime()).toBeGreaterThanOrEqual(currentTime.getTime() - 5000);
    expect(new Date(savedRequest?.createdAt as Date).getTime()).toBeLessThanOrEqual(new Date().getTime());
    
    // Check populated fields
    expect((savedRequest?.requesterId as any).fullname).toBe(requester.fullname);
