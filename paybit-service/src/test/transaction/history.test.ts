...
beforeAll(async () => {
  ...
  await User.deleteMany({ email: { $in: [sender.email, receiver.email, unrelatedUser.email] } });
  ...
  const hashedUnrelatedPassword = await bcrypt.hash(unrelatedUser.password, salt);
...
describe('Transaction History Controller', () => {
  describe('GET /api/transaction/history', () => {
    ...
    it('should handle complex combining of multiple filters correctly', async () => {
      const response = await request(app)
        .get('/api/transaction/history?type=payment&status=completed&minAmount=50&maxAmount=200&sort=amount-low')
        .set('x-auth-token', senderAuthToken)
        .expect(200);

      const amounts = response.body.data.transactions.map((tx: any) => tx.amount);
    ...
    it('should handle invalid sort parameters gracefully', async () => {
      ...
      const dates = response.body.data.transactions.map((tx: any) => new Date(tx.date).getTime());
    ...
    it('should handle pagination correctly', async () => {
      ...
      expect(secondPage.body.data.transactions[0].id).not.toBe(response.body.data.transactions[0].id);
    });
    ...
    it('should include campaign ID for donation transactions', async () => {
      ...
      expect(response.body.data.transactions).toHaveLength(1);
      expect(response.body.data.transactions[0]).toHaveProperty('campaignId', campaignId);
    });

    it('should apply multiple filters together', async () => {
      const response = await request(app)
        .get('/api/transaction/history?type=payment&direction=sent&minAmount=100')
        .set('x-auth-token', senderAuthToken)
        .expect(200);
