...
const testCampaign = {
  ...
  description: 'This campaign is created specifically to test the deletion functionality. It needs to be at least 20 characters long.',
  goalAmount: 1000,
...
const campaignWithDonations = {
  ...
  description: 'This campaign already has some donations and will test the deletion behavior for campaigns with collected funds.',
  goalAmount: 2000,
  collectedAmount: 500,
...
beforeAll(async () => {
  ...
  await DonationCampaign.deleteMany({ name: { $in: [testCampaign.name, campaignWithDonations.name] } });
...
describe('Donation - Delete Campaign Controller', () => {
  ...
  it('should successfully delete a campaign that has donations', async () => {
    // Verify campaign exists before deletion
    const campaignBefore = await DonationCampaign.findById(campaignWithDonationsId);
    expect(campaignBefore).toBeTruthy();
    expect(campaignBefore?.collectedAmount).toBe(campaignWithDonations.collectedAmount);
    ...
    const campaignAfter = await DonationCampaign.findById(campaignWithDonationsId);
    expect(campaignAfter).toBeNull();
  });
  ...
  it('should return error when user is not authenticated', async () => {
    ...
    const newCampaign = await DonationCampaign.create({
      ...
      description: 'This campaign is used to test deletion without authentication.',
      creatorUid: creator.uid,
      goalAmount: 1000,
      collectedAmount: 0,
      image: ''
  ...
  it('should handle server errors gracefully', async () => {
    ...
    const errorTestCampaign = await DonationCampaign.create({
      ...
      description: 'This campaign is used to test error handling during deletion.',
      creatorUid: creator.uid,
      goalAmount: 1000,
      collectedAmount: 0,
      image: ''
    ...
    DonationCampaign.findByIdAndDelete = jest.fn().mockImplementationOnce(() => {
  ...
  it('should handle non-existent user gracefully', async () => {
    ...
    const tempCampaign = await DonationCampaign.create({
      ...
      description: 'This is a temporary campaign for testing non-existent user deletion.',
