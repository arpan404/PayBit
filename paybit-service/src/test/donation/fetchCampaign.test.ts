...
const testCampaigns = [
    {
        ...
        description: 'This is a campaign with a high goal amount for testing sorting and filtering.',
    ...
    },
    {
        ...
        description: 'This is a campaign with a low goal amount for testing sorting and filtering.',
        creatorUid: testUser.uid,
        goalAmount: 500,
        collectedAmount: 250,
    ...
    },
    {
        ...
        description: 'This campaign is created with a more recent timestamp for sorting testing.',
        creatorUid: testUser.uid,
        goalAmount: 1500,
        collectedAmount: 300,
    ...
    },
    {
        ...
        description: 'This campaign is created with an older timestamp for sorting testing.',
        creatorUid: testUser.uid,
        goalAmount: 1500,
        collectedAmount: 600,
    ...
    },
    {
        ...
        description: 'This campaign has a high progress percentage for testing progress sorting.',
        creatorUid: testUser.uid,
        goalAmount: 1000,
        collectedAmount: 900,
    ...
    },
    {
        ...
        description: 'This campaign contains a unique term - UNIQUESEARCHTERM - for testing search.',
        creatorUid: testUser.uid,
        goalAmount: 2000,
        collectedAmount: 100,
...
beforeAll(async () => {
    ...
    for (const campaign of [testCampaigns[0], testCampaigns[1], testCampaigns[4], testCampaigns[5]]) {
    ...
    const otherUserCampaign = await DonationCampaign.create({
        ...
        description: 'This campaign is created by another user for testing creator filtering.',
...
describe('Donation - Fetch Campaign Controller', () => {
    ...
    it('should fetch a single campaign by ID', async () => {
        ...
        expect(response.body).toHaveProperty('message', 'Campaign retrieved successfully');
    ...
    it('should fetch all campaigns with default pagination', async () => {
        ...
        // Check response structure
        expect(response.body).toHaveProperty('success', true);
        expect(response.body).toHaveProperty('message', 'Campaigns retrieved successfully');
        expect(response.body).toHaveProperty('data');
    ...
    it('should filter campaigns by creator UID', async () => {
        ...
        const otherUserCampaignIncluded = campaigns.some((c: { id: string; }) => c.id === otherUserCampaignId);
    ...
    it('should filter campaigns by goal amount range', async () => {
        ...
        const lowGoalCampaignIncluded = campaigns.some((c: { name: string; }) => c.name === 'Low Goal Campaign');
        expect(lowGoalCampaignIncluded).toBe(false);

        // Should not include the high goal campaign (10000)
        const highGoalCampaignIncluded = campaigns.some((c: { name: string; }) => c.name === 'High Goal Campaign');
    ...
    it('should sort campaigns by newest first (default)', async () => {
        ...
        const newestCampaignIndex = campaigns.findIndex((c: { name: string; }) => c.name === 'Newest Campaign');
    ...
    it('should sort campaigns by oldest first', async () => {
        ...
        const oldestCampaignIndex = campaigns.findIndex((c: { name: string; }) => c.name === 'Oldest Campaign');
    ...
    it('should sort campaigns by goal amount (high to low)', async () => {
        ...
        for (let i = 0; i < campaigns.length - 1; i++) {
            expect(campaigns[i].goalAmount).toBeGreaterThanOrEqual(campaigns[i + 1].goalAmount);
        }
    ...
    it('should sort campaigns by progress percentage', async () => {
        ...
        for (let i = 0; i < campaigns.length - 1; i++) {
            const currentProgress = campaigns[i].collectedAmount / campaigns[i].goalAmount;
            const nextProgress = campaigns[i + 1].collectedAmount / campaigns[i + 1].goalAmount;
            expect(currentProgress).toBeGreaterThanOrEqual(nextProgress);
        }

        // The high progress campaign should be first or among the first
        const highProgressIndex = campaigns.findIndex((c: { name: string; }) => c.name === 'High Progress Campaign');
    ...
    it('should paginate results correctly', async () => {
        ...
        const firstPageIds = firstPageResponse.body.data.campaigns.map((c: { id: any; }) => c.id);
        ...
        const secondPageIds = secondPageResponse.body.data.campaigns.map((c: { id: any; }) => c.id);
    ...
    it('should apply multiple filters together', async () => {
        const response = await request(app)
            .get(`/api/donation/campaign?creatorUid=${testUser.uid}&minGoal=1000&sort=goal-low`)
            .expect(200);
        ...
        for (let i = 0; i < campaigns.length - 1; i++) {
            expect(campaigns[i].goalAmount).toBeLessThanOrEqual(campaigns[i + 1].goalAmount);
        }
    });
