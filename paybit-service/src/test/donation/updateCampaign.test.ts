...
const testCampaign = {
  ...
  description: 'This is the original description that is long enough to meet the minimum requirements.',
  goalAmount: 1000,
  collectedAmount: 200,
...
const validUpdates = {
  ...
  description: 'This is the updated description which is still long enough to meet the minimum requirements and includes some additional details.',
  goalAmount: 1500,
...
beforeAll(async () => {
  ...
  await DonationCampaign.deleteMany({ name: { $in: [testCampaign.name, validUpdates.name] } });
...
afterAll(async () => {
  await User.deleteMany({ email: { $in: [creator.email, otherUser.email] } });
  await DonationCampaign.deleteMany({ creatorUid: { $in: [creator.uid, otherUser.uid] } });
});
...
describe('Donation - Update Campaign Controller', () => {
  ...
  it('should successfully update all fields of a campaign', async () => {
    ...
    expect(response.body).toHaveProperty('message', 'Campaign updated successfully');
  ...
  it('should successfully update a single field', async () => {
    ...
    expect(response.body.data).toHaveProperty('description', testCampaign.description);
    expect(response.body.data).toHaveProperty('goalAmount', testCampaign.goalAmount);
  ...
  it('should successfully update the goal amount above the collected amount', async () => {
    ...
    expect(response.body.data).toHaveProperty('goalAmount', goalUpdate.goalAmount);
    
    // Verify progress is calculated correctly
    const expectedProgress = (testCampaign.collectedAmount / goalUpdate.goalAmount) * 100;
  ...
  it('should return error when goal amount is less than collected amount', async () => {
    ...
    await request(app)
      ...
      .expect((res) => {
        ...
        expect(res.body.message).toContain('Goal amount cannot be less than the already collected amount');
      });
  });
  ...
  it('should handle server errors gracefully', async () => {
    ...
    DonationCampaign.findByIdAndUpdate = jest.fn().mockImplementationOnce(() => {
