...
const testCampaign = {
  ...
  description:
    "This campaign is created specifically for testing donations. The description is long enough to meet requirements.",
  goalAmount: 1000,
  collectedAmount: 200,
...
describe("Donation - Donate Controller", () => {
  ...
  it("should successfully process a donation", async () => {
    ...
    expect(data).toHaveProperty(
      "newTotal",
      testCampaign.collectedAmount + donationAmount,
    );
    ...
    expect(updatedCampaign?.collectedAmount).toBe(
      testCampaign.collectedAmount + donationAmount,
    );
  });
  ...
  it("should return error when donating to own campaign", async () => {
    ...
    const ownCampaign = await DonationCampaign.create({
      name: "Own Campaign",
      description:
        "This is a campaign created by the donor to test self-donation prevention.",
  ...
  it("should handle non-existent campaign creator", async () => {
    ...
    const nonExistentCreatorCampaign = await DonationCampaign.create({
      ...
      description:
        "This campaign has a non-existent creator UID to test error handling.",
  ...
  it("should correctly calculate progress and completion status", async () => {
    ...
    const almostCompleteCampaign = await DonationCampaign.create({
      ...
      description:
        "This campaign is already almost at its goal to test completion logic.",
      creatorUid: creator.uid,
      goalAmount: 100,
      collectedAmount: 90,
      image: "",
