 ...
 * @access Private
 */
export const fetchRequests = async (req: ExpressRequest, res: Response): Promise<void> => {
  try {
    ...
    if (type && !sender) {
      ...
      res.status(400).json({
        success: false,
        code: 'fetch-request-e3',
        message: "Missing required query param. Use 'sender=me' or 'type=sent/received'."
      });
      return;
    }
    ...
    res.status(200).json({
      success: true,
      message: isSenderMe ? 'Sent requests retrieved' : 'Received requests retrieved',
      data: {
